...
1. Run this command to create the files, folders and the needed dependencies for the React application.
```js
...
Make sure you're even into the folder and run:
```js
npx create-react-app .
```
This dot `'.'` means you want to create the react applications `here` into this folder.
...
## Step 4
### Once into the folder, just call the VS Code IDE application by the command
```js
code .
```
![Screenshot](./img/step4.1.png)\
Just to remember, this dot `'.'` requires this specific folder for exploring their files.

![Screenshot](./img/step4.2.png)
As you can see your project is open and brings things created by React, but at first the most important is the folder `scr` (source) for starting coding.
For seeing it works, go back to GitBash terminal and run:
```js
npm start
```
...
`Blue`: Well, it's a very well intuitive message: your project is in the browser.\
`Green`: Local, means your project is available on your PC machine and can be got by the address `http://localhost:3000`.

![Screenshot](./img/step4.5.png)

`Yellow`: On the other hand, your project is also available for other devices since on the same network and can be got by the address `http://192.168.0.108:3000`. This IP changes according to your network configurations.
